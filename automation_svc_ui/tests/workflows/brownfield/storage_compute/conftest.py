...
from automation_svc_ui.tests.workflows.brownfield.networking.existing_acct_new_devices.brnf_network_existing_acct_new_devices import \
    WfExistingAcctNewDevices
from automation_svc_ui.tests.workflows.brownfield.storage_compute.brnf_str_com_existing_acct_devices import (
...
@pytest.fixture(scope="class")
def order_sc_storage_baas_devices():
    create_test = WfScExistingAcctDevices()
    device_test = "STORAGE"
    for i in range(2):
        mfr_storage_dev = create_test.existing_acct_sc_create_device_fn(device_test, baas=True)
        if mfr_storage_dev:
            return mfr_storage_dev
        else:
            time.sleep(3)
