...
@pytest.mark.xfail
class TestMoveDeviceFromParentToChildCustomerSvc:
    ...
    def test_move_device_from_parent_to_child_customer_c1220657(self, adi_app_api_session):
        ...
        iap_devices = VerifyByApi.get_all_network_devices_by_pcid(adi_app_api_session=adi_app_api_session,
                                                                  pcid=parent_pcid)
        ...
        moveTo_response = adi_app_api_session.move_device_bw_parent_child(operation="moveTo", device_list=devices,
                                                                          pcid=child_pcid)

        assert moveTo_response.status_code == 200 and adi_app_api_session.verify_device_claimed_by_pcid(
            platform_customer_id=child_pcid, device_serial_number=device_serial)

        # move back device to parent customer
        moveFrom_response = adi_app_api_session.move_device_bw_parent_child(operation="MoveFrom", device_list=devices,
                                                                            pcid=child_pcid)

        assert moveFrom_response.status_code == 200 and adi_app_api_session.verify_device_claimed_by_pcid(
            platform_customer_id=parent_pcid, device_serial_number=device_serial)
