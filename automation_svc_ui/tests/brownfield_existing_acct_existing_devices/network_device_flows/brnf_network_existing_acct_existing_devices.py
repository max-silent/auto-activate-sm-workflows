...
from automation_svc_ui.local_libs.activate_inventory.activate_device_ui_utils import DeviceHistoryVerifier
...
from local_libs.audit_logs.audit_logs_ui_utils import AuditLogsVerifier, LogsEventType
from local_libs.ui_doorway.ui_doorway_devices_by_api import UiDoorwayDevices
...
class WfExistingAcctExistingDevices:
    def __init__(self):
        ...
        self.pcid_name = ExistingUserAcctDevices.test_data[
            "brnf_existing_acct_existing_devices_acct1"
        ]
    ...
    def wf_sm_activate_folder_add_rule_check(
            self, browser_instance, storage_state_path, folder_name
    ):
        ...
        try:
            ...
            ManageAccount(
                page, self.cluster
            ).open_activate().side_menu.navigate_to_folders()
            ActivateFolders(
                page, self.cluster
            ).wait_for_loaded_table().open_folder_details(folder_name)
            ActivateFolderDetails(page, self.cluster).add_new_rule(
                self.rule
            ).should_have_rule_in_the_list(self.rule.name).delete_rule(self.rule.name)
            return True
    ...
    def check_already_claimed_device_upload_via_csv(self, browser_instance, storage_state_path):
        ...
        try:
            ...
            DevicesInventory(page, self.cluster).search_for_text(
                claimed_iap_serial_no, ensure_not_empty=False
            ).should_have_rows_count(0).search_for_text(
                claimed_sw_serial_no, ensure_not_empty=False
    ...
    def check_claimed_device_remains_claimed(self, browser_instance, storage_state_path):
        ...
        try:
            ...
            ).should_have_rows_count(1).search_for_text(
                ExistingUserAcctDevices.test_data[
                    "brnf_existing_acct_existing_devices_sw_serial_subs_mgmt"
                ]
            ).should_have_rows_count(
    ...
    def brnf_sa_device_est_prov(
            self, device_type, serial_number, mac_address, part_number
    ):
        """
    ...
    def brnf_sa_switch_device_prov(
            self, device_type, serial_number, mac_address, part_number, api_path, x_type
    ):
        """
        To make /verifyestchallenge for VGW/NONTPM device
