...
@allure.suite("Brownfield_network_devices - service_centric_ui")
@allure.sub_suite("Brownfield_existing_account_existing_network_devices")
class TestExistingAccountExistingDevicesEstSvc:
    ...
    def test_brnf_sa_vgw_est_provision(self, brnf_sa_vgw_est_prov):
        ...
        assert brnf_sa_vgw_est_prov.headers.get('X-Status-Code') == expected_response_data["est_provision_success"][
            "X-Status-Code"]
    ...
    def test_brnf_sa_vgw_verify_est_provision(self, brnf_sa_vgw_verify_est):
        ...
        assert brnf_sa_vgw_verify_est.headers.get('X-Status-Code') == expected_response_data["est_provision_success"][
            "X-Status-Code"]
    ...
    def test_brnf_sa_nontpm_est_provision(self, brnf_sa_nontpm_est_prov):
        ...
        assert res.get('cert') is not None
        assert res.get('estUrl') is not None
        assert brnf_sa_nontpm_est_prov.headers.get('X-Status-Code') == expected_response_data["est_provision_success"][
            "X-Status-Code"]
        assert brnf_sa_nontpm_est_prov.headers.get('X-Authenticated') == \
               expected_response_data["est_provision_success"][
                   "X-Authenticated"]
    ...
    def test_brnf_sa_nontpm_verify_est_provision(self, brnf_sa_nontpm_verify_est):
        ...
        assert brnf_sa_nontpm_verify_est.headers.get('X-Status-Code') == \
               expected_response_data["est_provision_success"]["X-Status-Code"]
        assert brnf_sa_nontpm_verify_est.headers.get('X-Authenticated') == \
               expected_response_data["est_provision_success"][
                   "X-Authenticated"]
        assert res.get("success") == expected_response_data["est_provision_success"]["success"]
    ...
    def test_brnf_sa_nontpm_device_provision_success(self, brnf_sa_nontpm_prov):
        ...
        assert brnf_sa_nontpm_prov.headers['X-Status-Code'] in (
        expected_response_data["switch_provision_success"]["X-Status-Code"],
    ...
    def test_brnf_sa_nontpm_device_provision_unsuccess(self, brnf_sa_nontpm_prov_invalid_mac):
        ...
        """
        assert brnf_sa_nontpm_prov_invalid_mac.status_code == expected_response_data["switch_provision_no_device"][
            "response_code"]
        assert brnf_sa_nontpm_prov_invalid_mac.headers['X-Status-Code'] == \
               expected_response_data["switch_provision_no_device"]["X-Status-Code"]
    ...
    def test_nontpm_sw_dev_firmware(self, existing_acct_existing_dev_brnf_sa_sw_firmware, api_path, x_type):
        ...
        """
        assert existing_acct_existing_dev_brnf_sa_sw_firmware.status_code == \
               expected_response_data['switch_firmware_response']['response_code']
        assert existing_acct_existing_dev_brnf_sa_sw_firmware.headers["X-Status-Code"] == \
               expected_response_data['switch_firmware_response'][
                   'X-Status-Code']
