...
def pytest_configure(config):
    testrail_configs["test_run"] = config.option.TESTRAIL_RUN_NAME
    testrail_configs["child_milestone"] = config.option.CHILD_MILESTONE
...
def pytest_addoption(parser):
    ...
    parser.addoption("--run_name", dest="TESTRAIL_RUN_NAME", action="store", default=testrail_configs["test_run"])
    parser.addoption("--milestone", dest="CHILD_MILESTONE", action="store", default=testrail_configs["child_milestone"],
                     help="Argument to set Child Milestone to specific value 'UndefTestType', "
                          "so test results would not be populated to TestRail")
...
if os.getenv("POD_NAMESPACE") is None:
    exec(f'from automation.configs.local_tb_data.{current_env}_settings import {current_env}_test_data')
else:
    exec(f'from automation.configs.pipeline_tb_data.{current_env}_settings import {current_env}_test_data')

# global variables used for aop flows
aop_dict = {'manufacture': [], 'license': [], 'sds': [], 'pos': []}
pytest.globalvar = {'runtime_testdata': {'STORAGE': aop_dict, 'NETWORK': aop_dict, 'COMPUTE': aop_dict}}
...
class Creds:
    ...
    def downloaded_test_envs():
        ...
        try:
            ...
            log.error("not able to download the test_envs files")
...
def get_add_device_expected_responses():
    data_file = os.path.join(AUTOMATION_DIRECTORY, "configs", "activate_device_direct_test_data", "add_test_data.json")
...
def adi_app_api_session(app_api_key=None, app_api_secret=None):
    api_key = ExistingUserAcctDevices.test_data["brnf_existing_acct_existing_devices_client_id"],
    api_secret = ExistingUserAcctDevices.test_data["brnf_existing_acct_existing_devices_client_secret"],
...
class SubscriptionData:
    def __init__(self, device_type):
        subscription_data = {"IAP": {"subs_type": "CENTRAL_AP", "part_number": "JW242AR"},
