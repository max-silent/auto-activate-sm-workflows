...
@pytest.fixture(scope="session")
def brnf_sa_msp_login_load_account():
    hostname = ExistingUserAcctDevices.login_page_url
...
@pytest.fixture(scope="class")
def rma_app_api_session():
    """
    ===== Creates an Authorization bearer token with scope rma ======
    """
    rma_app_api_session = ActivateInventory(
        host=ExistingUserAcctDevices.app_api_hostname,
        sso_host=ExistingUserAcctDevices.sso_hostname,
        client_id=ExistingUserAcctDevices.test_data[
            "rma_devices_client_id"
        ],
        client_secret=ExistingUserAcctDevices.test_data[
            "rma_devices_client_secret"
        ],
        scope="rma"
    )
    return rma_app_api_session
