...
from automation.local_libs.activate_devices.manufacture_order_utils import ManufactureDevicesHelper
...
class WfExistingAcctNewDevices:
    ...
    def wf_applications_page(self, browser_instance, storage_state_path) -> bool:
        """Step #21: Check Applications page title."""
        log.info(f"Playwright: verifying logged in user at Users page.")
        ...
        try:
            ...
            applications_page.should_have_text_in_title("My Applications")
    ...
    def wf_subscriptions_page(self, browser_instance, storage_state_path, devices_details) -> bool:
        """Step #22: Check Device Subscription."""
        ...
        try:
            ChooseAccount(page, self.cluster).open() \
                .go_to_account_by_name(self.pcid_name) \
                .wait_for_loaded_state().nav_bar.navigate_to_manage()
            device_subscriptions = ManageAccount(page, self.cluster).open_subscriptions().wait_for_loaded_table()
    ...
    def wf_assign_subs_eval(self, device_type, ordered_devices, brnf_sa_login_load_account):
        ...
        query_params = {"limit": 100, "expire_date_cut_off_in_millis": expire_date_cut_off}
        try:
            for i in range(1, 10):
        ...
        license_key = None
        for index in range(len(license_list["subscriptions"])):  # top loop
            for key, value in license_list["subscriptions"][index].items():  # nested loop 1
                if key == "subscription_type" and value == subs_type:
                    if subs_type == "CENTRAL_AP":
                        for k, v in license_list["subscriptions"][index].items():  # nested loop 2 (AP)
                            if k == "license_tier" and v == "ADVANCED":
                                subs_details = license_list["subscriptions"][index]
                    ...
                    if subs_type == "CENTRAL_SWITCH":
                        for k, v in license_list["subscriptions"][index].items():  # nested loop 2 (SW)
                            if k == "license_tier" and v == "ADVANCED":
                                for k1, v1 in license_list["subscriptions"][index].items():  # nested loop 3 (SW)
                                    if k1 == "subscription_tier_description" and v1 in ["Advanced-Switch-62xx/29xx",
                                                                                        "Advanced-Switch-6200/29xx"]:
                                        subs_details = license_list["subscriptions"][index]
                                        if self._is_actual_subs(subs_details):
                                            license_key = subs_details["subscription_key"]
                                            log.info(license_key)
                                            break  # exit from nested loop 3 (SW)
                            if license_key:
                                break  # exit from nested loop 2 (SW)
                    if subs_type == "CENTRAL_GW":
                        for k, v in license_list["subscriptions"][index].items():  # nested loop 2 (GW)
                            if k == "license_tier" and v == "ADVANCED":
                                for k1, v1 in license_list["subscriptions"][index].items():  # nested loop 3 (GW)
                                    if k1 == "subscription_tier_description" and v1 in ["Advance-70XX",
                                                                                        "Advanced-70xx/90xx"]:
                                        subs_details = license_list["subscriptions"][index]
                                        if self._is_actual_subs(subs_details):
                                            license_key = subs_details["subscription_key"]
                                            log.info(license_key)
                                            break  # exit from nested loop 3 (GW)
                            if license_key:
                                break  # exit from nested loop 2 (GW)
                    if license_key:
                        break  # exit from nested loop 1
            if license_key:
                break  # exit from top loop
        if not license_key:
        ...
        for app in provisioned_apps["provisions"]:
            if app["provision_status"] == "PROVISIONED":
                ...
                if len(sm_app_subs_by_pcid["subscriptions"]) >= 14:
                    log.info("subscriptions list by sm_app_subs_by_pcid: {}".format(
                            len(sm_app_subs_by_pcid["subscriptions"])
                        )
                    )
                    return True
                else:
