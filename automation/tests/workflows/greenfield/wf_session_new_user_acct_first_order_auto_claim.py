                ...
                if key == "subscription_type" and value == subs_type:
                    ...
                    if subs_type == "CENTRAL_SWITCH":
                        for k, v in license_list["subscriptions"][index].items():  # nested loop 2 (SW)
                            if k == "license_tier" and v == "ADVANCED":
                                for k1, v1 in license_list["subscriptions"][
                                    index
                                ].items():  # nested loop 3 (SW)
                                    if (
                                            k1 == "subscription_tier_description"
                                            and v1 in ["Advanced-Switch-62xx/29xx", "Advanced-Switch-6200/29xx"]
                                    ):
                                        subs_details = license_list["subscriptions"][index]
                                        if cls._is_actual_subs(subs_details):
                                            license_key = subs_details["subscription_key"]
                    ...
                    if subs_type == "CENTRAL_GW":
                        for k, v in license_list["subscriptions"][index].items():  # nested loop 2 (GW)
                            if k == "license_tier" and v == "ADVANCED":
                                for k1, v1 in license_list["subscriptions"][
                                    index
                                ].items():  # nested loop 3 (GW)
                                    if (
                                            k1 == "subscription_tier_description"
                                            and v1 in ["Advance-70XX", "Advanced-70xx/90xx"]
                                    ):
                                        subs_details = license_list["subscriptions"][index]
                                        if cls._is_actual_subs(subs_details):
                                            license_key = subs_details["subscription_key"]
