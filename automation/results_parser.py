...
def get_test_id_from_allure_result_dict(source):
    """
    This method parsed test's result tags to find test case id (like 'C123123' or 'testrail(id=123123))
...
def create_result_testrail_report(path):
    ...
    for file in get_list_of_results_files(path):
        test_results = prepare_testrail_results_from_allure_json(get_json_from_file(os.path.join(path, file)))
...
def write_down_report(report, result_path):
    ...
    try:
        ...
        with open(f"{result_path}/{report_name}", "w") as f:
            ...
            log.info(f"TestRail report results was saved to '{result_path}/{report_name}' file successfully.")
...
if __name__ == "__main__":
    ...
    write_down_report(create_result_testrail_report(final_results_path), final_report_path)
